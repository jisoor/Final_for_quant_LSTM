...
updated_df = pd.read_pickle('./models/last_60_nikkei.pkl')    # 얘는 이제 주석 처리
# updated_df = pd.read_pickle('./models/updated_df_nikkei.pickle')  # datasets폴더 안에 updated_df 데이터프레임 저장해서 늘 최신 데이터 불러오게 하기
updated_df.rename(columns={'Close':'Price'}, inplace=True)
...
df_Nikkei = pd.DataFrame(investpy.indices.get_index_recent_data('Nikkei 225','japan' ))
print('닠케이', df_Nikkei.head())
df_Nikkei = df_Nikkei[['Close']]
print('종가만', df_Nikkei.head())
df_Nikkei = df_Nikkei[start_day:Yesterday] # 01-19부터 어제꺼까지. 기존 행의 마지막 날짜로 부터 하루 뒤부터 어제 까지 데이터를 인베스팅 닷컴엣서 끌어옴.
df_Nikkei.rename(columns={'Close':'Price'}, inplace=True)
print(df_Nikkei.head())
print(df_Nikkei.tail())
print(df_Nikkei.index)


updated_df = pd.concat([updated_df, df_Nikkei])
print('업데이트된 df :', updated_df.tail(10))

updated_df.to_pickle('./models/updated_df_nikkei.pickle')  # 업데이트된 DataFrame 저장시키기(나중에 다시 불러올 데이터)
