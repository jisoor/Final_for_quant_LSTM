...
model = load_model('./models/crude_oil_model_15.h5')

# 마지막 30개 데이터 예측 시키기 ,  30개의 데이터가 필요
raw_data = pd.read_csv('./datasets/Clear_Crude_Oil_Data_15.csv')
...
last_test_data.info()
# last_test_data.to_pickle("last_60.pkl")
# exit()
################ 이렇게 안하고 그냥, last_data = raw_data[-60:][['Price']] 해도 됨.

# minmaxscaler 어떻게 파일 가져오지??
with open('./models/minmaxscaler_oil_15.pickle', 'rb') as f:
...
sequence_test_X = []
sequence_test_Y = []
for i in range(len(scaled_last_test_data)-16):# 30개 데이타
  x = scaled_last_test_data[i:i+15]
  y = scaled_last_test_data[i+15]
...
######### 내일 값을 예측 시켜보자
tomorrow_predict = model.predict(scaled_last_test_data[-15:].reshape(1, 15, 1)) #마지막 30개 리셰잎 (30,1)짜리가 1개 있다.
