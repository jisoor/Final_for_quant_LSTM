...
data = raw_data[:-15][['Price']] #마지막 30개는 벡테스팅용, 'Price'열만 가져와서
print(data.head(5))
...
with open('./models/minmaxscaler_oil_15.pickle', 'wb') as f:
    pickle.dump(minmaxscaler, f)

for i in range(len(scaled_data)-15): #3515번 돌아감 : 3575 - 30 - 15 + 1
    _x = scaled_data[i:i+15] # i, i+1 ... i+ 29 까지 총 30개
    _y = scaled_data[i+15] # i + 30번째를 예측
...
scaled_data_15 = X_train, X_test, Y_train, Y_test

np.save('./models/crude_oil_scaled_data_15.npy', scaled_data_15)
